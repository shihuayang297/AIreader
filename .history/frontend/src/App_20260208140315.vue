...
// 中间阅读区选中文本气泡菜单：翻译/批判/润色 → 右侧智能体
const handleAiActionFromBubble = (payload) => {
    updateLastActionTime()
    isAiSidebarOpen.value = true
    const agentId = payload.agentId || 'encyclopedia'
    nextTick(() => {
        rightSidebarRef.value?.handleExternalRequest({ type: 'action', agent: agentId, prompt: payload.message })
    })
}
    ...
    <div class="flex-1 min-w-0 bg-[#eef2f6] relative flex flex-col z-10 items-center">
        <div class="w-full h-full">
            <MiddlePdfReader 
                ...
                @ai-action="handleAiActionFromBubble"
